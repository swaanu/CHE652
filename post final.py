...
for b in B:
    for a in A:
        for d in D:
            hrs_required = labour_hours[b][d]
            if hrs_required > 0:
                labourers = ceil(hrs_required / (5 * 8))
                manufacturing_schedule[(d, b, a)] = (labourers, 5)

# === Assembly labour ===
assembly_schedule = {}
for a in A:
    for d in D:
        hrs_required = System_Integration[d]
        if hrs_required > 0:
            labourers = ceil(hrs_required / (5 * 8))
            assembly_schedule[(d, a)] = (labourers, 5)
...
for a in A:
    if assembly_cont[a].X > 0.5:  # assembly chosen
        print(f"\nAssembly Location: {a}")
# Assign values for manufacturing
for (d, b, a), (labourers, days) in manufacturing_schedule.items():
    num_labours[d, b, a].Start = labourers
    num_days[d, b, a].Start = days

# Assign values for assembly
for (d, a), (labourers, days) in assembly_schedule.items():
    num_labours_assembly[d, a].Start = labourers
    num_days_assembly[d, a].Start = days


print("\n--- Manufacturing Labour Plan for India ---")
for (d, b, a), (labourers, days) in manufacturing_schedule.items():
    if a == "India":
        print(f"Part: {b:<30} | Type: {d:<15} | Labourers: {labourers:<3} | Days: {days}")
print("\n--- Assembly Labour Plan for India ---")
for (d, a), (labourers, days) in assembly_schedule.items():
    if a == "India" and labourers > 0:
        print(f"Type: {d:<15} | Labourers: {labourers:<3} | Days: {days}")
